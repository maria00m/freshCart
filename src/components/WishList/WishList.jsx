...
import LoadindScreen from "../LoadingScreen/LoadingScreen";
import toast from "react-hot-toast";
import axios from "axios";

function WishlistProduct({ product, productRemove }) {
  return (
    <div className="p-3 mb-4 border rounded-lg shadow-md w-[50%] sm:w-1/3 md:w-1/4 lg:w-1/5">
      <img src={product.imageCover} alt={product.image} className="w-full"/>
      <h2 className="text-xl font-semibold">{product.title.split(" ").slice(0, 2).join(" ")}</h2>
    ...
    <>
      {isLoading ? (
        ...
        <div className="mx-auto container px-4 md:px-6 2xl:px-0 py-12 flex flex-col">
          ...
          <div className=" flex flex-wrap gap-x-2 mt-4 justify-center">
