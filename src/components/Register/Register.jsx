  ...
  return (
    <>
      <div className="max-w-lg mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md px-8 py-10 flex flex-col items-center">
        <h1 className="text-xl font-bold text-center text-[#027d02] dark:text-gray-200 mb-8">Welcome to My Company</h1>
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
          ...
          <button type="submit" className="bg-[#027d02] hover:bg-green-600 text-white font-medium py-2 px-4 rounded-md shadow-sm disabled:bg-gray-400" disabled={isLoading}>Register 
...
{errorMsg && <p className='text-red-500'>{errorMsg}</p>}      
  {successMsg && <p className='text-green-500'>{successMsg}</p>}
        ...
        <div className="mt-4 text-center">
          ...
          <Link to="/login" className="text-[#027d02] hover:text-green-600">Login</Link>
        </div>
      </div>
    </>
  );
